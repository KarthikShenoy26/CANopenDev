 ...
 *
 * BIT MASK for checking the attributes of a OD objects.
 * canopen Object dictionary attribute(CO_ODA_XXXX_YYYY)
 */
typedef enum{
...
typedef struct{
    ...
    //DOMAIN data TYPE
		...
		uint32_t            offset;
}CO_ODF_arg_t;//CANopen object dictionary function argument type.
...
typedef struct{
    ...
    uint8_t            *flags;
}CO_OD_extension_t;//can open object dictionary extension type
...
}CO_SDO_t;//canopen sdo server type
...
}CO_bytes_t;//can open bytes type
