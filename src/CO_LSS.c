		 ...
		 switch(LSS->state)
		 {
			 ...
			 case CO_LSS_ST_INQUIRE_VENDOR_ID_RESP:
				 if(msg->data[0]==INQUIRE_VENDOR_ID_CSC)
				 {
					 ...
					 temp[0]=msg->data[1]; temp[1]=msg->data[2]; temp[2]=msg->data[3]; temp[3]=msg->data[4];
			 ...
			 case CO_LSS_ST_IDENTIFY_SLAVE_RESP:
				 if(msg->data[0]==IDENTITY_SLAVE_CSC)
				 {
					 LSS->LSSRespData.slaveCommandSpecifier=msg->data[0];
					 LSS->LSSRespData.CANrxNew=true;
				 }
        ...
        uint16_t                CANdevTxIdx)
{
	if(LEVEL_1){sprintf(logLine,
	    ...
	    LSS->CANtxBuff = CO_CANtxBufferInit(
	                 CANdevTx,               /* CAN device */
	    ...
	    if(LEVEL_1){sprintf(logLine,
	    			"FILE: CO_LSS.c"
	    			"||CALL: CO_LSS_init"
	    			"\nMSG: Init successful"); logPrint(ERROR,logLine);}
	        return CO_ERROR_NO;
}
        ...
        uint16_t               *timerNext_ms)
{
	if(LEVEL_1){sprintf(logLine,
	 ...
	 CO_LSS_state_t state = CO_LSS_ST_IDLE;
	 ...
	 /* return if idle */
	     if(LSS->state==CO_LSS_ST_IDLE){
	     	if(LEVEL_1){sprintf(logLine,
	     			"FILE: CO_LSS.c"
	     			"||CALL: CO_LSS_process"
	     			"\nMSG: No new msg recved, and LSS Master in Idle state"); logPrint(LOG,logLine);}
	         return NO_RESPONSE;
	     }
	     ...
	     if(!NMTisStopped){
	       	if(LEVEL_1){sprintf(logLine,
	       			"FILE: CO_LSS.c"
	       			"||CALL: CO_LSS_process"
	       			"\nMSG: This node is Not in stopped mode. LSS Communication is not support in this mode."); logPrint(LOG,logLine);}
	     ...
	     case CO_LSS_ST_SWITCH_MODE_SELECTIVE_REQ:
																...
																//SERIAL NUMBER
																   //fill data into the CANtxBuffer
																	...
																	state=CO_LSS_ST_SWITCH_MODE_SELECTIVE_RESP;
	    	 break;

	     case CO_LSS_ST_SWITCH_MODE_SELECTIVE_RESP:
															 if((LSS->LSSRespData.isRespTimeout==false)&& (LSS->LSSRespData.CANrxNew==true))
															 {
																 state=CO_LSS_ST_IDLE;
																 LSS->LSSRespData.CANrxNew=false;
	     ...
	     case CO_LSS_ST_CONFIGURE_NODE_ID_REQ:
																	 ...
																	 CO_CANsend(LSS->CANdevTx, LSS->CANtxBuff);
																	 //No response to be received so switch to idle state.
																	 state=CO_LSS_ST_CONFIGURE_NODE_ID_RESP;
	    	 break;
	     ...
	     case CO_LSS_ST_CONFIGURE_BIT_TIMING_PARAM_REQ:
																 ...
																 CO_CANsend(LSS->CANdevTx, LSS->CANtxBuff);
																 //No response to be received so switch to idle state.
																 state=CO_LSS_ST_CONFIGURE_BIT_TIMING_PARAM_RESP;
	    	 break;
//--TO DO
	     case CO_LSS_ST_CONFIGURE_BIT_TIMING_PARAM_RESP:
															 if((LSS->LSSRespData.isRespTimeout==false)&& (LSS->LSSRespData.CANrxNew==true))
	     ...
	     case CO_LSS_ST_STORE_CONFIGURATION_REQ:
															 ...
															 CO_CANsend(LSS->CANdevTx, LSS->CANtxBuff);
															 //No response to be received so switch to idle state.
															 state=CO_LSS_ST_STORE_CONFIGURATION_RESP;
	    	 break;
...
//--TO DO
	     ...
	     case CO_LSS_ST_INQUIRE_VENDOR_ID_REQ:
														 ...
														 CO_CANsend(LSS->CANdevTx, LSS->CANtxBuff);
														 //No response to be received so switch to idle state.
														 state=CO_LSS_ST_INQUIRE_VENDOR_ID_RESP;
	    	 break;
...
//--TO DO
	     ...
	     case CO_LSS_ST_INQUIRE_PRODUCT_ID_REQ:
														 ...
														 CO_CANsend(LSS->CANdevTx, LSS->CANtxBuff);
														 //No response to be received so switch to idle state.
														 state=CO_LSS_ST_INQUIRE_PRODUCT_ID_RESP;
	    	 break;
...
//--TO DO
	     ...
	     case CO_LSS_ST_INQUIRE_REVISION_ID_REQ:
														 ...
														 CO_CANsend(LSS->CANdevTx, LSS->CANtxBuff);
														 //No response to be received so switch to idle state.
														 state=CO_LSS_ST_INQUIRE_REVISION_ID_RESP;
	    	 break;
...
//--TO DO
	     ...
	     case CO_LSS_ST_INQUIRE_SERIAL_NUM_REQ:
														 ...
														 CO_CANsend(LSS->CANdevTx, LSS->CANtxBuff);
														 //No response to be received so switch to idle state.
														 state=CO_LSS_ST_INQUIRE_SERIAL_NUM_RESP;
	    	 break;
...
//--TO DO
	     ...
	     case CO_LSS_ST_IDENTITY_REMOTE_SLAVE_REQ:
														...
														//VENDOR ID
															 //fill data into the CANtxBuffer
																 //Command specifiers
																 LSS->CANtxBuff->data[0]=IDENTITY_REMOTE_SLAVE_VENDOR_ID_CSC;
																 CO_setUint32(temp,LSS->LSSTxdata.vendorID);
																 LSS->CANtxBuff->data[1]=temp[0];
																 LSS->CANtxBuff->data[2]=temp[1];
																 LSS->CANtxBuff->data[3]=temp[2];
													  ...
													  //PRODUCT CODE
															 //fill data into the CANtxBuffer
																 //Command specifiers
																 LSS->CANtxBuff->data[0]=IDENTITY_REMOTE_SLAVE_PRODUCT_CODE_CSC;
																 CO_setUint32(temp,LSS->LSSTxdata.productCode);
																 LSS->CANtxBuff->data[1]=temp[0];
																 LSS->CANtxBuff->data[2]=temp[1];
																 LSS->CANtxBuff->data[3]=temp[2];
													...
													//REVISION NUMBER LOW
															 //fill data into the CANtxBuffer
																 //Command specifiers
																 LSS->CANtxBuff->data[0]=IDENTITY_REMOTE_SLAVE_REVISION_NUM_LOW_CSC;
												   ...
												   //REVISION NUMBER HIGH
															 //fill data into the CANtxBuffer
															 //Command specifiers
															 LSS->CANtxBuff->data[0]=IDENTITY_REMOTE_SLAVE_REVISION_NUM_HIGH_CSC;
												  ...
												  //SERIAL NUMBER LOW
															 //fill data into the CANtxBuffer
															 //Command specifiers
															 LSS->CANtxBuff->data[0]=IDENTITY_REMOTE_SLAVE_SERIAL_NUM_LOW_CSC;
												 ...
												 //SERIAL NUMBER HIGH
															 //fill data into the CANtxBuffer
															 //Command specifiers
															 LSS->CANtxBuff->data[0]=IDENTITY_REMOTE_SLAVE_SERIAL_NUM_HIGH_CSC;
															 ...
															 //No response to be received so switch to idle state.
														 state=CO_LSS_ST_IDENTIFY_SLAVE_RESP;
	    	 break;
